...
# This script builds a Siamese "pairs" CSV from Kaggle Pascal-VOC annotations
# It uses self-supervised positives (same bbox twice) and sampled negatives (different image_id)
# No image cropping is written to disk; downstream dataloaders can crop using bbox fields.
...
def build_pairs_for_split(rows, k_neg_per_pos=1, rng=None):
    ...
    # negatives: sample a different image's bbox for each r, k times
    by_image = index_by_image(rows)
    image_ids = list(by_image.keys())

    for r in rows:
        # candidate negatives are any rows whose image_id != r["image_id"]
        # choose k_neg_per_pos negatives
        for _ in range(k_neg_per_pos):
            # try up to a few times to sample a different image
            for _trial in range(10):
                other_img = rng.choice(image_ids)
                if other_img != r["image_id"]:
                    break
            # pick a box from that image
...
def main():
    ...
    ap.add_argument("--annotations_csv", type=str, default="data/kaggle/annotations.csv")
    ap.add_argument("--out_pairs_csv", type=str, default="data/kaggle/pairs_kaggle_ssl.csv")
