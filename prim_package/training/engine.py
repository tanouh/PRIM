...
def validate_triplet(
    ...
    for batch in loader:
        ...
        # Forward - pass text dictionary directly
        za = model.forward_once(a_img, a_txt)
        zp = model.forward_once(p_img, p_txt)
        zn = model.forward_once(n_img, n_txt)
...
def train_triplet_multimodal(
    ...
    for batch in loader:
        ...
        # Pass the text dictionary directly - model handles tokenization
        za = model.forward_once(a_img, a_txt)
        zp = model.forward_once(p_img, p_txt)
        zn = model.forward_once(n_img, n_txt)
...
@torch.no_grad()
def validate_contrastive_multimodal(
    model,
    loader: DataLoader,
    device: torch.device,
    distance: str = "cosine",
    ...
    """
    Validate multimodal model with contrastive loss by:
      - reporting mean distances for positive and negative pairs
      - optionally saving per-pair predictions to CSV.
    ...
    for batch in loader:
        # PairImageDataset (multimodal) returns:
        #   Length 7: (img1, txt1, img2, txt2, y, p1, p2)
        #   Length 5: (img1, txt1, img2, txt2, y)
        if len(batch) == 7:
            img1, txt1, img2, txt2, y, p1_list, p2_list = batch
...
@torch.no_grad()
def validate_triplet_multimodal(
    model,
    loader: DataLoader,
    device: torch.device,
    distance: str = "cosine",
    ...
    """
    Validate multimodal model with triplet loss by reporting mean d(a,p) and d(a,n),
    and optionally save per-triplet logs.
    ...
    for batch in loader:
        # TripletImageDataset (multimodal) returns:
        #   Length 9: (a_img, a_txt, p_img, p_txt, n_img, n_txt, a_paths, p_paths, n_paths)
        #   Length 6: (a_img, a_txt, p_img, p_txt, n_img, n_txt)
        if len(batch) == 9:
            a_img, a_txt, p_img, p_txt, n_img, n_txt, a_paths, p_paths, n_paths = batch
            have_paths = True
        else:
        ...
        # Forward pass with multimodal inputs - pass text dictionary directly
        za = model.forward_once(a_img, a_txt)
        zp = model.forward_once(p_img, p_txt)
        zn = model.forward_once(n_img, n_txt)
